...
public class SbMod implements ModInitializer {
	...
	public static String version = "v3.0.4.2-1.19.2";
	...
	public void onInitialize() {
		...
		try	{
			...
			if (motListe.isEmpty()){
				...
				myList = new BufferedReader(new FileReader("sbmod/skychatwords.txt")); // Chemin pour le dictionnaire
				Line = myList.readLine(); // Chaque ligne contient un mot
				while (Line != null){
					motListe.add(Line); // ajout du mot dans la liste
					Line = myList.readLine();
				}
				for (int i = 0; i < motListe.size(); i++) {
					String unsorted = motListe.get(i); // Mot qui n'est pas trie en ordre alphabetique
					char[] str = unsorted.toCharArray(); // Separation du mot en caratere
					Arrays.sort(str); // Triage du mot en ordre alphabetique
					String sorted = new String(str); // Mot qui est trie en ordre alphabetique
					if (motListe != null) {
						mapwords.put(sorted, unsorted); // mise dans le hashmap du mot trie (Key) et du mot normal (Value)
					}
				}
			}
